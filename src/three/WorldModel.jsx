...
// and the procedural textures (sillares, metal cepillado, roca, madera) give
// them surface detail instead of flat plastic.
const Tex = createContext({})

function Box({ position, args, color = '#e7dcc4', rotation, metalness = 0, tex }) {
  const t = useContext(Tex)[tex]
  return <RoundedBox position={position} rotation={rotation} args={args} radius={0.025} smoothness={2} castShadow receiveShadow><meshStandardMaterial map={t} bumpMap={t} bumpScale={t ? 1 : 0} color={color} roughness={0.65} metalness={metalness} /></RoundedBox>
}
function Cone({ position, args, color, tex }) {
  const t = useContext(Tex)[tex]
  return <mesh position={position} castShadow receiveShadow><coneGeometry args={args} /><meshStandardMaterial map={t} bumpMap={t} bumpScale={t ? 1.5 : 0} color={color} roughness={0.85} flatShading={!!t} /></mesh>
}
function Cylinder({ position, args, color, rotation, metalness = 0, tex }) {
  const t = useContext(Tex)[tex]
  return <mesh position={position} rotation={rotation} castShadow receiveShadow><cylinderGeometry args={args} /><meshStandardMaterial map={t} bumpMap={t} bumpScale={t ? 1 : 0} color={color} roughness={t === undefined ? 0.48 : 0.7} metalness={metalness} /></mesh>
}

export default function WorldModel(props) {
  const tex = useMemo(() => ({
    bricks: texture('bricks', 1),
    metal: texture('metal', 1),
    rock: texture('rock', 1),
    wood: texture('wood', 1),
    sand: texture('sand', 1),
  }), [])
  return <Tex.Provider value={tex}><Modelo {...props} /></Tex.Provider>
}

function Modelo({ kind, color, glow, locked }) {
  const accent = locked ? '#84958e' : color
  switch (kind) {
    case 'castle':
      return <group>
        <Box position={[0, -0.2, 0]} args={[1, 0.65, 0.7]} tex="bricks" color="#ffffff" />
        <Box position={[0, -0.3, 0.365]} args={[0.23, 0.43, 0.035]} color="#7a5a3a" tex="wood" />
        {[-1, 1].flatMap(x => [-1, 1].map(z => <group key={`${x}${z}`} position={[x * 0.45, 0, z * 0.32]}>
          <Cylinder args={[0.17, 0.2, 0.9, 16]} color="#ffffff" tex="bricks" />
          <Cone position={[0, 0.62, 0]} args={[0.25, 0.4, 16]} color={accent} />
          <Box position={[0, 0.16, 0.17]} args={[0.07, 0.18, 0.025]} color="#42505a" />
        </group>))}
        {[-0.3, 0, 0.3].map(x => <Box key={x} position={[x, 0.2, 0.32]} args={[0.16, 0.18, 0.15]} tex="bricks" color="#ffffff" />)}
    ...
    case 'rocket':
      return <group rotation={[0, 0, -0.15]}>
        <Cylinder args={[0.25, 0.3, 0.85, 32]} color="#ffffff" metalness={0.45} tex="metal" />
    ...
    case 'mountain':
      return <group>{[[-0.38, -0.22, 0.1, 0.65], [0.12, 0, -0.12, 1], [0.48, -0.26, 0.2, 0.55]].map(([x,y,z,s], i) => <group key={i} position={[x,y,z]} scale={s}>
        <Cone args={[0.6, 1.25, 7]} color={i === 1 ? '#a3aaae' : '#c7c1b6'} tex="rock" />
        <Cone position={[0, 0.43, 0]} args={[0.19, 0.4, 7]} color="#f1f4ed" />
      </group>)}</group>
    case 'crystal':
      return <group>
        <Cylinder position={[0, -0.12, 0]} args={[0.25, 0.7, 0.9, 11]} color="#8a817a" tex="rock" />
    ...
    case 'maze':
      return <group rotation={[0, 0.3, 0]}>
        <Box position={[0, -0.35, 0]} args={[1.25, 0.12, 1.1]} color="#e8dcc0" tex="sand" />
    ...
    case 'island':
      return <group>
        <Cylinder position={[0, -0.4, 0]} args={[0.65, 0.55, 0.16, 32]} color="#ffffff" tex="sand" />
        <group rotation={[0, 0, -0.14]}>
          <Cylinder position={[0, 0, 0]} args={[0.055, 0.09, 0.85, 10]} color="#c79a6a" tex="wood" />
        ...
        <Box position={[0.34, -0.25, 0.15]} rotation={[0, -0.4, 0]} args={[0.26, 0.22, 0.22]} color="#d9a765" tex="wood" />
      </group>
    case 'pizza':
