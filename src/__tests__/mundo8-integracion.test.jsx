...
// Verificación de integración del bucle jugable del Mundo 8: cruza contenido +
...
describe('integración: Mundo 8 jugable', () => {
  const world = findWorld('feria-datos')

  it('worlds exporta el Mundo 8 y findWorld lo resuelve', () => {
    expect(world).toBeTruthy()
    expect(worlds.map((w) => w.slug)).toContain('feria-datos')
    expect(world.id).toBe('mundo8')
  })

  it('tiene 6 niveles bien formados (briefing no vacío, reto con pick 3 y ≥3 fábricas)', () => {
    expect(world.levels).toHaveLength(6)
